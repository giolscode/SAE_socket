...
int main(int argc, char *argv[]) {
    ...
    int socketEcoute, clientX, clientO, observateurs[MAX_OBSERVATEURS];
    int nbObservateurs = 0;
    ...
    while (1) {
        ...
        int nouvelObservateur = accept(socketEcoute, (struct sockaddr *)&sockaddrDistant, &longueurAdresse);
        if (nouvelObservateur > 0) {
            if (nbObservateurs < MAX_OBSERVATEURS) {
                observateurs[nbObservateurs++] = nouvelObservateur;
                send(nouvelObservateur, "observe", strlen("observe") + 1, 0);  // Informe l'observateur
            } else {
                close(nouvelObservateur);  // Si trop d'observateurs, on ferme la connexion
            }
        }
        ...
        for (int i = 0; i < nbObservateurs; i++) {
            if (verifierVictoire(morpion, symboleEnJeu)) {
                ...
                if (send(observateurs[i], messageEnvoye, strlen(messageEnvoye) + 1, 0) <= 0) {
                    perror("Erreur d'envoi à un observateur");
                    close(observateurs[i]);
                    observateurs[i] = observateurs[nbObservateurs - 1];
                    nbObservateurs--;
                }
            }
            // Ne pas envoyer "end" aux observateurs, ils ne jouent pas
            else if (grillePleine(morpion)) {
                printf("Match nul !\n");
                break;
            }
            // Envoi uniquement des mises à jour sans messages de fin
            else {
                send(observateurs[i], "observe", strlen("observe") + 1, 0);
            }
        }
        ...
        if (verifierVictoire(morpion, symboleEnJeu)) {
            ...
            for (int i = 0; i < nbObservateurs; i++) {
                send(observateurs[i], messageEnvoye, strlen(messageEnvoye) + 1, 0);
            }
        ...
        // Dans le cas si la grille est pleine
            ...
            // Notification aux observateurs
            for (int i = 0; i < nbObservateurs; i++) {
                send(observateurs[i], "end", strlen("end") + 1, 0);
            }
        ...
        // Notification aux observateurs
            for (int i = 0; i < nbObservateurs; i++) {
                send(observateurs[i], "game_over", strlen("game_over") + 1, 0);
            }
    }
